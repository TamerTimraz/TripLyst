...
export function ItineraryCard({ title, destination, imageUrl, duration, author, likes, saves }: ItineraryCardProps) {
  return (
    <Card className="group overflow-hidden bg-card hover:shadow-md transition-all duration-300 border border-border/50">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-sm font-medium drop-shadow-sm">{author.name}</span>
          </div>
          <Button
            size="icon"
            variant="ghost"
      ...
      <div className="p-5 space-y-3">
        <div>
          <h3 className="font-semibold text-lg leading-tight text-balance text-foreground mb-2">{title}</h3>
